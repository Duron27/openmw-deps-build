...
NDK_VERSION="r29"
NDK_PATH="android-ndk-$NDK_VERSION"
...
cp -r $PROJECT_ROOT/ports . 2>/dev/null || echo "  No ports directory to copy"
cp -r $PROJECT_ROOT/triplets . 2>/dev/null || echo "  No triplets directory to copy"
cp -r $PROJECT_ROOT/vcpkg.json . 2>/dev/null || echo "  No vcpkg.json to copy"
